...
<Tabs groupId="ros-version">
  <TabItem value="ros2" label="ROS2">
    :::warning
    If you are following this tutorial after the [GPS Navigation Tutorial](outdoor.mdx) please remove the outdoor `MAVROSPY` container:
  ...
  <TabItem value="ros1" label="ROS1">
    ...
    On your publishing computer, start by opening the `mocap` Docker container:
    ...
    Next, export the `ROS_MASTER_URI` environment variable. Make sure to replace `master_ip_address` with the IP address of the companion computer:
