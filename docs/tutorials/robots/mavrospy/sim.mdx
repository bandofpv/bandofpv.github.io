...
First, start by downloading [Isaac Sim 4.2](https://docs.omniverse.nvidia.com/isaacsim/latest/index.html) which is the latest version supported by Pegasus Simulator extension which is required to integrate a PX4 backend and thus MAVROSPY. Currently, the traditional method of installing Isaac Sim via the Omniverse Launcher is not supported and must be downloaded manually:
